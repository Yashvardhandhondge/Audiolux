...
export async function POST(req:NextRequest){
    try{
        ...
        //A function for checking if the user is not the craetor
        if(user.id !== data.creatorId){
            ...
            //A function for checking duplicate song in the last 10 minutes
            ...
            // A check for rate limitings checks for non-creator user
        ...
        return NextResponse.json(
            {message : `Eroor while adding a stream ${e}`},
            {status : 500}
        )
    }
}
